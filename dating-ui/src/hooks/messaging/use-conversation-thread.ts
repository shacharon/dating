...
import { appendUniqueMessages } from '@/hooks/messaging/message-list-utils';
...
/** Thread REST/state/poll + conversation-scoped messaging socket. */
export function useConversationThread(
  ...
  }, [conversationId, enabled, detailCopy.loadMessagesFailed]);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        const { messages: incoming } = await fetchConversationMessages(
          conversationId,
          {
            after: lastId,
            limit: 100,
  ...
  const sendMessage = useCallback(
    ...
    [conversationId, detailCopy.sendFailed],
  );
