...
type PrismaSync = Pick<PrismaClient, 'profileExtractionV2' | 'profileSignalSnapshot'>;
 ...
 */
export async function syncProfileHgRankingSignalColumns(prisma: PrismaSync, profileId: string): Promise<void> {
  ...
  const nextInterests =
    selfSnap.hgRankingInterestsTop.length === 0 ? fallback : [...selfSnap.hgRankingInterestsTop];
