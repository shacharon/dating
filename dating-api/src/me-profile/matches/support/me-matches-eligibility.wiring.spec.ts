...
describe('me-matches-eligibility harness wiring', () => {
  it('split modules exist; spec-support stays a thin barrel', () => {
    ...
    const barrel = fs.readFileSync(
      path.join(supportDir, 'me-matches-eligibility.spec-support.ts'),
      'utf8',
    );
    expect(barrel).not.toMatch(/class EligibilityTestHarness/);
    expect(barrel).not.toMatch(/buildEligibilityPrismaMock/);
    expect(barrel).toMatch(/from '\.\/me-matches-eligibility\.harness'/);
  });

  it('e2e specs keep importing matches/support eligibility barrel', () => {
    for (const name of E2E_SPECS) {
      const content = fs.readFileSync(path.join(e2eDir, name), 'utf8');
