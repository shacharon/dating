...
import { createMeMatchesServiceForTest } from '../matches/support/me-matches.test-harness';
...
describe('MATCH_ENGINE_V1_CONTRACT (docs + runtime shape)', () => {
  ...
  describe('MeMatchesService read-model import policy (V1)', () => {
    const scoringSrc = [
      fs.readFileSync(
        path.join(__dirname, '../matches/core/me-matches.service.ts'),
        'utf8',
      ),
      fs.readFileSync(
        path.join(__dirname, '../matches/list/ranking/match-ranking.service.ts'),
        'utf8',
      ),
      fs.readFileSync(
        path.join(__dirname, '../matches/detail/match-detail-query.service.ts'),
        'utf8',
      ),
    ].join('\n');
