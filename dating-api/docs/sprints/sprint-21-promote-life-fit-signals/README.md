...
**Duration:** ~1–2 weeks (3 stories)
**Goal:** `conflictStyle` becomes an active 15th compatibility signal, and a new bounded `interestAlignment` term joins the score blend. Match scores are recalibrated so the change is intentional and measured, not accidental drift. The Holy Grail eligibility gate is untouched.
**Status:** Planned
...
| HG five-signal ranker deletion | **Out of scope / follow-up.** Removing the now-redundant `holy-grail-five-signal-ranking.ts` pipeline is a separate cleanup once this ships and is verified. |
...
- [ ] `conflictStyle` is an active numeric compatibility signal (extraction → scoring → explainability), not a shadow key.
- [ ] `interestAlignment` contributes a bounded, named term to the blended score; blend weights sum to 1.
- [ ] HG eligibility gate behavior is byte-for-byte unchanged (verified by its integration suite).
- [ ] Match scores recalibrated; golden-pair expectations updated deliberately with documented before/after deltas.
- [ ] Full `dating-api` test suite green (unit + integration).
- [ ] Backfill and HG-ranker deletion captured as follow-up tickets (not done here).
