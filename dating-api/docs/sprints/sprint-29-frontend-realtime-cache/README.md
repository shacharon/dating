...
**Status:** ✅ **Done** — Stories 1–5 complete  
...
| 05 | [Lazy-load admin / heavy UI](./STORY_05_lazy_admin_ui.md) | **Done** |
...
- [x] Product conversation thread does not 3s-poll messages when WS is available / defaulted on
- [x] Conversations list is cursor-paginated; badge can use unread-total without full inbox
- [x] Shared client cache (TanStack) for at least conversations / auth-adjacent hot paths Architect locks
- [x] Profile/match images use optimized `next/image` where Architect scopes
- [x] Admin (or other heavy) routes code-split via dynamic import
