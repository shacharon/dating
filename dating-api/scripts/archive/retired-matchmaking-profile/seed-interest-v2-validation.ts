...
/**
 * Seeds 40 synthetic profiles (synthetic-it-v2-001 … 040) with aboutMe/aboutPartner phrases
 * targeting interestTags v1+v2 extraction. Same HG fixture shape as `seed-lifestyle-v2-validation.ts`.
 *
 * Run: npm run seed:interest-v2-validation
...
export const INTEREST_V2_VALIDATION_ID_PREFIX = 'synthetic-it-v2-';

const PREFIX = INTEREST_V2_VALIDATION_ID_PREFIX;
const COUNT = 40;
  ...
  try {
    for (let i = 1; i <= COUNT; i++) {
      ...
      const textHash = createHash('sha256')
        .update(`${aboutMe}|${aboutPartner}|itv2`, 'utf8')
        .digest('hex')
        .slice(0, 16);
      ...
      await prisma.profileEvaluation.upsert({
        where: { profileId: id },
        create: { profileId: id, evaluatedAt: new Date(), promptVersion: 'itv2', policyVersion: 'itv2', textHash },
      ...
      const interestsTop3 = [`itv2_${i}`, 'hiking', 'music'];
      ...
      await prisma.profileExtractionV2.upsert({
        ...
        create: {
          profileId: id,
          promptVersion: 'itv2_fixture',
          textHash,
          extractionJson: { itv2: true, profileId: id },
          selfSignals: {},
          partnerSignals: {},
          relationshipSignals: {},
          coverageScore: 1,
    ...
    console.error(
      `seed-interest-v2-validation: upserted ${COUNT} profiles (${PREFIX}001–${String(COUNT).padStart(3, '0')})`,
    );
  } finally {
