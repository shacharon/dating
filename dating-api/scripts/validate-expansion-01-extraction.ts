/**
 * Optional live LLM validation for Expansion-01 shadow signals.
 * Compares self-domain extraction against curated fixture bands.
 *
 * Run: npm run validate:expansion-01-extraction
...
const FIXTURES_PATH = join(ROOT, 'data', 'expansion-01-extraction-fixtures.json');
const AGREEMENT_THRESHOLD = 0.85;

interface Expansion01Fixture {
  id: string;
  aboutMe: string;
  signal: 'empathyCompassion' | 'vulnerabilityOpenness';
  expectedMin: number;
  expectedMax: number;
}
...
async function main(): Promise<void> {
  ...
  const fixtures = JSON.parse(raw) as Expansion01Fixture[];
  ...
  console.log(
    `\nExpansion-01 extraction agreement: ${(agreement * 100).toFixed(1)}% (${passes}/${scored} scored fixtures)`,
  );
