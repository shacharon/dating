/**
 * Optional live LLM validation for Expansion-05 shadow signals.
 * Compares self-domain extraction against curated fixture bands.
 *
 * Run: npm run validate:expansion-05-extraction
...
const FIXTURES_PATH = join(ROOT, 'data', 'expansion-05-extraction-fixtures.json');
const AGREEMENT_THRESHOLD = 0.85;

interface Expansion05Fixture {
  id: string;
  aboutMe: string;
  signal: 'physicalActivityLevel' | 'domesticComfort';
  expectedMin: number;
  expectedMax: number;
}
...
async function main(): Promise<void> {
  ...
  const fixtures = JSON.parse(raw) as Expansion05Fixture[];
  ...
  console.log(
    `\nExpansion-05 extraction agreement: ${(agreement * 100).toFixed(1)}% (${passes}/${scored} scored fixtures)`,
  );
