/**
 * Optional live LLM validation for Expansion-10 shadow signals.
 * Compares self-domain extraction against curated fixture bands
 * (EN high/low/null + Hebrew + distinction cases).
 *
 * Run: npm run validate:expansion-10-extraction
...
function flattenExpectations(
  fixture: Expansion10Fixture,
): Expansion10Expectation[] {
...
async function main(): Promise<void> {
  ...
  const fixtures = JSON.parse(raw) as Expansion10Fixture[];
  ...
  console.log(
    `\nExpansion-10 extraction agreement: ${(agreement * 100).toFixed(1)}% (${passes}/${scored} scored expectations)`,
  );
