/**
 * Optional live LLM validation for Expansion-13 shadow signals.
 * Compares self-domain extraction against curated fixture bands
 * (EN high/low/null + Hebrew + distinction cases).
 *
 * Run: npm run validate:expansion-13-extraction
...
function flattenExpectations(
  fixture: Expansion13Fixture,
): Expansion13Expectation[] {
...
async function main(): Promise<void> {
  ...
  const fixtures = JSON.parse(raw) as Expansion13Fixture[];
  ...
  console.log(
    `\nExpansion-13 extraction agreement: ${(agreement * 100).toFixed(1)}% (${passes}/${scored} scored expectations)`,
  );
