/**
 * Optional live LLM validation for Expansion-15 shadow signals.
 * Compares self-domain extraction against curated fixture bands
 * (EN high/low/null + Hebrew + distinction cases).
 *
 * Run: npm run validate:expansion-15-extraction
...
const FIXTURES_PATH = join(ROOT, 'data', 'expansion-15-extraction-fixtures.json');
...
function flattenExpectations(
  fixture: Expansion15Fixture,
): Expansion15Expectation[] {
...
async function main(): Promise<void> {
  ...
  const fixtures = JSON.parse(raw) as Expansion15Fixture[];
  ...
  console.log(
    `\nExpansion-15 extraction agreement: ${(agreement * 100).toFixed(1)}% (${passes}/${scored} scored expectations)`,
  );
