/**
 * Optional live LLM validation for Expansion-06 shadow signal.
 * Compares self-domain extraction against curated fixture bands.
 *
 * Run: npm run validate:expansion-06-extraction
...
const FIXTURES_PATH = join(ROOT, 'data', 'expansion-06-extraction-fixtures.json');
const AGREEMENT_THRESHOLD = 0.85;

interface Expansion06Fixture {
  id: string;
  aboutMe: string;
...
async function main(): Promise<void> {
  ...
  const fixtures = JSON.parse(raw) as Expansion06Fixture[];
  ...
  console.log(
    `\nExpansion-06 extraction agreement: ${(agreement * 100).toFixed(1)}% (${passes}/${scored} scored fixtures)`,
  );
