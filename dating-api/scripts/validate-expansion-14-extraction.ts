/**
 * Optional live LLM validation for Expansion-14 shadow signals.
 * Compares self-domain extraction against curated fixture bands
 * (EN high/low/null + Hebrew + distinction cases).
 *
 * Run: npm run validate:expansion-14-extraction
...
const FIXTURES_PATH = join(ROOT, 'data', 'expansion-14-extraction-fixtures.json');
...
function flattenExpectations(
  fixture: Expansion14Fixture,
): Expansion14Expectation[] {
...
async function main(): Promise<void> {
  ...
  const fixtures = JSON.parse(raw) as Expansion14Fixture[];
  ...
  console.log(
    `\nExpansion-14 extraction agreement: ${(agreement * 100).toFixed(1)}% (${passes}/${scored} scored expectations)`,
  );
