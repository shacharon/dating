/**
 * Optional live LLM validation for Expansion-04 shadow signals.
 * Compares self-domain extraction against curated fixture bands.
 *
 * Run: npm run validate:expansion-04-extraction
...
const FIXTURES_PATH = join(ROOT, 'data', 'expansion-04-extraction-fixtures.json');
const AGREEMENT_THRESHOLD = 0.85;

interface Expansion04Fixture {
  id: string;
  aboutMe: string;
  signal: 'intellectualCuriosity' | 'creativeExpression';
  expectedMin: number;
  expectedMax: number;
}
...
async function main(): Promise<void> {
  ...
  const fixtures = JSON.parse(raw) as Expansion04Fixture[];
  ...
  console.log(
    `\nExpansion-04 extraction agreement: ${(agreement * 100).toFixed(1)}% (${passes}/${scored} scored fixtures)`,
  );
