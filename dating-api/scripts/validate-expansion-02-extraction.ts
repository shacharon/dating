/**
 * Optional live LLM validation for Expansion-02 shadow signals.
 * Compares self-domain extraction against curated fixture bands.
 *
 * Run: npm run validate:expansion-02-extraction
...
const FIXTURES_PATH = join(ROOT, 'data', 'expansion-02-extraction-fixtures.json');
const AGREEMENT_THRESHOLD = 0.85;

interface Expansion02Fixture {
  id: string;
  aboutMe: string;
  signal: 'emotionalRegulation' | 'physicalAffectionStyle';
  expectedMin: number;
  expectedMax: number;
}
...
async function main(): Promise<void> {
  ...
  const fixtures = JSON.parse(raw) as Expansion02Fixture[];
  ...
  for (const fixture of fixtures) {
    ...
    if (score == null) {
      console.log(`FAIL ${fixture.id}: null (expected ${fixture.expectedMin}-${fixture.expectedMax})`);
      continue;
    }
    scored += 1;
    const ok =
      score >= fixture.expectedMin && score <= fixture.expectedMax;
    if (ok) {
      passes += 1;
  ...
  console.log(
    `\nExpansion-02 extraction agreement: ${(agreement * 100).toFixed(1)}% (${passes}/${scored} scored fixtures)`,
  );
