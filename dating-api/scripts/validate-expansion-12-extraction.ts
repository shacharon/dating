/**
 * Optional live LLM validation for Expansion-12 shadow signals.
 * Compares self-domain extraction against curated fixture bands
 * (EN high/low/null + Hebrew + distinction cases).
 *
 * Run: npm run validate:expansion-12-extraction
...
const FIXTURES_PATH = join(ROOT, 'data', 'expansion-12-extraction-fixtures.json');
...
function flattenExpectations(
  fixture: Expansion12Fixture,
): Expansion12Expectation[] {
...
async function main(): Promise<void> {
  ...
  const fixtures = JSON.parse(raw) as Expansion12Fixture[];
  ...
  console.log(
    `\nExpansion-12 extraction agreement: ${(agreement * 100).toFixed(1)}% (${passes}/${scored} scored expectations)`,
  );
