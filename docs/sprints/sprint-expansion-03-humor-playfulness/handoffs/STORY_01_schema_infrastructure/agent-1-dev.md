...
- Added `humorPlayfulness` to **`SHADOW_SIGNAL_KEYS`**.
- Bumped `MAX_EVIDENCE_ITEMS` **30 → 31** (15 official + 12 shadow + 4 buffer).
- Updated unit tests: 12 shadow / 27 total; Expansion-03 shadow-mode regression block.
...
| `dating-api/src/extraction/extracted-signals.spec.ts` | Counts + Expansion-03 regression tests |
...
- Shadow allowlist only — key **not** in `COMPATIBILITY_SIGNAL_KEYS` / `SignalKey`
- Expansion-01/02 keys unchanged
...
**Notes for CR:** Confirm only allowlist files changed; key not in scored arrays.
